...
func (r Running) Calories() float64 {
	...
	return ((float64(CaloriesMeanSpeedMultiplier)*meanSpeed + CaloriesMeanSpeedShift) * r.Training.Weight / float64(MInKm) * float64(r.Training.Duration.Hours()) * float64(MinInHours))
}
...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Training.Weight + (math.Pow(meanSpeed, 2)/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Training.Weight) * float64(w.Training.Duration.Hours()) * MinInHours)
}
...
func (s Swimming) Calories() float64 {
	...
	return (meanSpeed + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Training.Weight * float64(s.Training.Duration.Hours())
}

func (s Swimming) TrainingInfo() InfoMessage {
	infoMessage := s.Training.TrainingInfo()
	infoMessage.Speed = s.meanSpeed()
	infoMessage.Distance = infoMessage.Speed * float64(s.Training.Duration.Hours())
	return infoMessage
}
